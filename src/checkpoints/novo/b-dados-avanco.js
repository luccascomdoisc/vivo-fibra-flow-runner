...
/**
 * [INFINITY] Checkpoint B - Dados pessoais: CPF e data de nascimento (segunda
  ...
  try {
    ...
    try {
      ...
      const detalhe = erros.length
        ? `Infinity; form invalido na etapa Dados: ${erros.join(', ')}`
        : 'Infinity; sem avanco apos Continuar compra (etapa Endereco nao apareceu)';
    ...
    return makeResult('ok', start, screenshotUrl, `Infinity; etapa Dados submetida (botao via ${viaBotao}); etapa Endereco na tela${viaApi}`);
  } catch (e) {
